...
export const updateBooking = async (id: string, booking: object): Promise<Booking> => {
    try {
...
export const cancelBookingPayment = async (bookingId: string): Promise<Booking> => {
    try {
...
export const getBookingsByCheckInDate = async (date: string): Promise<Booking[]> => {
    const response = await axiosInstance.get(`/bookings/check-in-date?date=${date}`);
    return response.data;
};

export const getBookingsByCheckInDateRange = async (startDate: string, endDate: string): Promise<Booking[]> => {
    const response = await axiosInstance.get(`/bookings/check-in-range?startDate=${startDate}&endDate=${endDate}`);
    return response.data;
};

export const getBookingsByCheckOutDate = async (date: string): Promise<Booking[]> => {
    const response = await axiosInstance.get(`/bookings/check-out-date?date=${date}`);
    return response.data;
};

export const getBookingsByCheckOutDateRange = async (startDate: string, endDate: string): Promise<Booking[]> => {
    const response = await axiosInstance.get(`/bookings/check-out-range?startDate=${startDate}&endDate=${endDate}`);
    return response.data;
};
...
export const addServicesToBooking = async (bookingId: string, items: BookingServiceCreateItem[]) => {
    const res = await api.post(`${ENDPOINT}/${bookingId}/services/bulk`, items);
    return res.data;
};

export const addServiceToBooking = async (bookingId: string, item: BookingServiceCreateItem) => {
    const res = await api.post(`${ENDPOINT}/${bookingId}/services`, item);
    return res.data;
};

// =============================================
export const checkRoomAvailability = async (roomNumber: string, checkInDate: string, checkOutDate: string): Promise<Booking[]> => {
    try {
        const response = await api.get(`${ENDPOINT}/check-availability`, { params: { roomNumber, checkInDate, checkOutDate } });
        return response.data;
    } catch (error) {
...
export const cancelBooking = async (bookingId: string, cancelReason: string, cancelledBy: string, refundMethod: any | null) => {
    try {
...
export const getBookingServicesByBookingId = async (bookingId: string) => {
    try {
        const response = await api.get(`${ENDPOINT}/booking-services/booking/${bookingId}`);
        return response.data;
    } catch (error) {
...
export const confirmPayAtCheckout = async (bookingId: string): Promise<Booking> => {
    try {
        const response = await api.put(`${ENDPOINT}/${bookingId}/confirm-pay-at-checkout`);
        return response.data;
    } catch (error) {
