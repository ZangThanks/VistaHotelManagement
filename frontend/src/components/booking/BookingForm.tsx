...
export default function BookingForm({
        ...
        try {
            ...
            if (bookingType === 'HOURLY') {
                try {
                    ...
                    console.error(
                        'Failed to fetch hourly rate policies:',
                        error,
                    );
                }
            }
            ...
            const roomPromises = roomsToUse.map((roomId) =>
                getRoomById(roomId),
            );
            ...
            if (customerId) {
                ...
                const custVoucher = await getByCustomerIdAndStateTrue(
                    customerId,
                );
    ...
    const isDateRangeOverlapping = (
        ...
        return bookedDates.some((bookedDate) => {
            ...
            return (
                bookedNormalized >= ciNormalized &&
                bookedNormalized < coNormalized
            );
        });
    };
        ...
        if (policy.baseRates) {
            ...
            if (
                ...
                if (rate !== undefined) {
                    ratePercentage = rate;
                    console.log(
                        `Base rate for ${duration}h: ${ratePercentage}%`,
                    );
                } else {
                    console.warn(
            ...
            // Fallback: nếu là array (cho tương thích)
                ...
                const matchedRate = sortedRates.find(
                    (r) => duration >= r.baseHours,
                );
        ...
        if (bookingType === 'HOURLY') {
            ...
            return rooms.reduce((sum, room) => {
                ...
                console.log(
                    `Room ${
                        room.roomNumber
                    }: ${basePrice} × ${ratePercentage}% = ${totalPrice.toFixed(
                        0,
                    )} VND`,
                );
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        {rooms.map((room) => {
                                                            const roomNumber =
                                                                room.roomNumber;
                                                            if (!roomNumber)
                                                                return null; // Skip if roomNumber is undefined
                        ...
                        {bookingType === 'DAILY' ? (
                            ...
                            <>
                                <div className="flex justify-between items-center py-2 border-t border-gray-200 mt-4">
                                    ...
                                    <span className="text-gray-900 font-medium">
                                        {hourlyCheckInDate?.toLocaleDateString(
                                            'en-GB',
                                        )}{' '}
                                        at {checkInTime}
                                    </span>
