          ...
          <div className="p-6">
            {availableVouchers.length === 0 ? (
              <div className="flex justify-center items-center py-12">
                <div className="text-gray-500">No vouchers available</div>
              </div>
            ) : (
              <div className="space-y-3">
                {availableVouchers.map((custVoucher, index) => {
                  const isChecked = selectedVoucherIds.includes(
                    custVoucher.voucher.voucherID
                  );
                  const discountType = custVoucher.voucher.discountType;

                  return (
                    <div
                      key={index}
                      onClick={() =>
                        toggleVoucher(custVoucher.voucher.voucherID)
                      }
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <h4 className="font-semibold text-gray-900">
                            {custVoucher.voucher.voucherName}
                          </h4>
                            ...
                            {discountType === "PERCENT" ? (
                              <>
                                ...
                                <span className="font-semibold text-[#CCBDA3]">
                                  {custVoucher.voucher.discountPercentage}%
                                </span>{" "}
                                of total
                              </>
                            ) : (
