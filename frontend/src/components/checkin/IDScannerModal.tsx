...
function IDScannerModal({
  ...
  useEffect(() => {
    if (isOpen) {
      console.log("Modal opened, getting cameras...");
      getCameras();
      resetState();
    } else {
      stopCamera();
  ...
  const processImage = async (imageSrc: string) => {
    ...
    try {
      ...
      const idInfo = parseIDCardInfo(extractedText);
  ...
  const parseIDCardInfo = (text: string): IDCardInfo | null => {
    try {
      ...
      for (const line of lines) {
        const match = line.match(idNumberPattern);
        if (match && !idNumber) {
          idNumber = match[1];
          break;
        }
      ...
      if (dobMatches) {
        dateOfBirth = dobMatches[0];
      }

      if (text.includes("Nam") || text.includes("Male")) {
        gender = "Nam";
      } else if (text.includes("Nữ") || text.includes("Female")) {
        gender = "Nữ";
      }

      const expiryMatches = Array.from(text.matchAll(expiryPattern));
      if (expiryMatches.length > 1) {
        expiryDate = expiryMatches[expiryMatches.length - 1][0];
      }
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2 flex-wrap">
                <select
                  ...
                  disabled={isScanning || availableCameras.length === 0}
                >
              ...
              >
                {!capturedImage && !isScanning && (
                  <>
                    {selectedCamera ? (
                      <Webcam
                        ...
                        onUserMediaError={(err) => {
                          ...
                          setError(
                            "Không thể khởi động camera: " + err.message
                          );
                        }}
                      />
                    ) : (
                    ...
                    {/* ✅ OVERLAY CHỈ HIỆN KHUNG VIỀN - KHÔNG CHE CAMERA */}
                    {selectedCamera && (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none p-8">
                        <div className="w-full h-full border-4 border-white/70 rounded-lg shadow-2xl"></div>
                      </div>
                    )}
                  </>
                )}
