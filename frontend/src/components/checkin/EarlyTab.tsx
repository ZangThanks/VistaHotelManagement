...
const EarlyTab = ({ onViewDetails }: EarlyTabProps) => {
  ...
  const handleApprove = async (id: string, status: "APPROVED" | "REJECTED") => {
    ...
    try {
      ...
      setRequests((prev) =>
        prev.map((req) =>
          req.requestID === id ? { ...req, approvalStatus: status } : req
        )
      );
    } catch (err) {
      console.error(err);
          ...
          {requests.map((req) => {
            ...
            return (
              ...
              >
                ...
                <td className="py-4 px-4">{formatTime(req.requestTime)}</td>
                    ...
                    {req.approvalStatus === "PENDING" && (
                      <>
                        <button
                          ...
                          onClick={() =>
                            handleApprove(req.requestID, "APPROVED")
                          }
                        ...
                        <button
                          disabled={processingId === req.requestID}
                          onClick={() =>
                            handleApprove(req.requestID, "REJECTED")
                          }
