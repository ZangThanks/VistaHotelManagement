...
const EarlyTab = ({ onViewDetails, bookings = [] }) => {
  const earlyCheckIns = bookings
    ...
    .map((booking, index) => {
      const requestId = `ER-${new Date().getFullYear().toString().slice(-2)}${(
        new Date().getMonth() + 1
      )
        .toString()
      ...
      return {
        ...
        requestedTime:
          ...
          booking.specialRequests.match(/(\d{1,2}:\d{2})/)[0]
            ? booking.specialRequests.match(/(\d{1,2}:\d{2})/)[0] + " AM"
            : "10:30 AM",
  ...
  if (earlyCheckIns.length === 0 && bookings.length > 0) {
    return (
      <div className="p-10 text-center">
        <p className="text-gray-500">No early check-in requests found.</p>
      </div>
    );
  }
      ...
      <table className="min-w-full bg-white">
        <thead>
          <tr className="bg-[#EBE3D7]/30 text-left">
            ...
            <th className="py-4 px-4 font-semibold">Regular Check-in</th>
            <th className="py-4 px-4 font-semibold">Requested Time</th>
        ...
        <tbody>
          {displayRequests.map((request) => (
            ...
            >
              ...
              <td className="py-4 px-4">{request.regularCheckIn}</td>
              <td className="py-4 px-4">{request.requestedTime}</td>
              <td className="py-4 px-4">{request.earlyFee}</td>
              <td className="py-4 px-4">{renderStatusBadge(request.status)}</td>
              <td className="py-4 px-4">
