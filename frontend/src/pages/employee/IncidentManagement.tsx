...
const IncidentManagement: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'incidents' | 'room-changes'>('incidents');
    ...
    const [roomChangeStatusFilter, setRoomChangeStatusFilter] = useState<'ALL' | 'PENDING' | 'APPROVED' | 'REJECTED'>('ALL');
    ...
    const loadRoomChangeRequests = async () => {
        ...
        try {
            ...
            const requests: RoomChangeRequest[] = storedRequests 
                ? JSON.parse(storedRequests)
                : [];
            
            console.log('📋 Loaded room change requests:', requests);
    ...
    const filteredRoomChanges = useMemo(() => {
        return roomChangeRequests.filter((request) => {
            const matchesSearch =
                ...
                request.currentRoomNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
                request.newRoomNumber.toLowerCase().includes(searchTerm.toLowerCase());

            const matchesStatus =
                roomChangeStatusFilter === 'ALL' || request.status === roomChangeStatusFilter;
    ...
    const stats = useMemo(() => {
        if (activeTab === 'incidents') {
            ...
            return {
                ...
                pending: roomChangeRequests.filter((r) => r.status === 'PENDING').length,
                approved: roomChangeRequests.filter((r) => r.status === 'APPROVED').length,
                rejected: roomChangeRequests.filter((r) => r.status === 'REJECTED').length,
            };
        }
    ...
    const handleRoomChangeAction = async (
        ...
        try {
            ...
            const updatedRequests = requests.map(req =>
                req.id === id
                    ? { ...req, status: action, responseNote: note }
                    : req
            );
            
            // Save back to localStorage
            localStorage.setItem('roomChangeRequests', JSON.stringify(updatedRequests));
            
            // Update local state
            setRoomChangeRequests(updatedRequests);
            ...
            <div className="bg-white border-b border-gray-200">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            onClick={activeTab === 'incidents' ? loadIncidents : loadRoomChangeRequests}
                            disabled={isLoading}
                    ...
                    <div className="flex gap-2 mt-6 border-b border-gray-200">
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                {stats.pending > 0 && activeTab === 'incidents' && (
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                {activeTab === 'room-changes' && stats.pending > 0 && (
                ...
                <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
                    <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex-1 relative">
                            ...
                            <input
                                type="text"
                                placeholder={activeTab === 'incidents' ? "Search by code, title, customer..." : "Search by code, customer, room..."}
                                value={searchTerm}
                                                        ...
                                                        <div className="flex items-center justify-center gap-2">
                                                            <button
                                                                onClick={() => {
                                                                    ...
                                                                    handleRoomChangeAction(request.id, 'APPROVED', note || undefined);
                                                                }}
                                                            ...
                                                            <button
                                                                onClick={() => {
                                                                    ...
                                                                    if (note) {
                                                                        handleRoomChangeAction(request.id, 'REJECTED', note);
                                                                    }
                                                                }}
